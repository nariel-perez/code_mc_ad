!--------------------------------------------------------------------
!  File: InputParams.f90      (Fortran 2003, compatible gfortran/ifort)
!--------------------------------------------------------------------
module InputParams
   !  Alias del tipo para evitar colisión con la variable global 'cell'
   ...
   !--- La celda global (tipo renombrado) ---------------------------
...
subroutine read_input(fname)
   ...
   do
      ...
      !--------------------------------------------------------------
      ! 1) recorta el comentario '!'  (coloca esto justo después de leer LINE)
      if (index(line,'!') > 0) line = line(:index(line,'!')-1)
      if (trim(line) == '') cycle        ! línea vacía → siguiente
      
      ! 2) INT primero (para 200, 10, 5000 ...)
      read(line, *, iostat=ios) key, ival
      if (ios == 0) then
         call set_int(lowercase(trim(key)), ival)
      else
         ! 3) REAL después (para 3.027, 1e0, 39.5 ...)
         read(line, *, iostat=ios) key, rval
         if (ios == 0) then
            call set_real(lowercase(trim(key)), rval)
         else
            call set_char(line)          ! por si es cadena (nam)
         end if
      end if
      !--------------------------------------------------------------
...
subroutine set_real(k,v)
   ...
   case ('angux','alpha','alfx','angx'); angux=v
   case ('anguy','beta','alfy','angy'); anguy=v
   case ('anguz','gamma','alfz','angz'); anguz=v
   case ('acel'); ACEL=v
